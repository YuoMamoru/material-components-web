...
| --- | --- | --- |
| `init()` | コンポーネントの初期がの準備が整ったとき、ホストクラスによって呼び出される | イベントリスナの追加、アダプタによる情報の問い合わせなど |
| `destroy()` | コンポーネントが使用されなくなったとき、ホストクラスによって呼び出される | イベントリスナの削除、一時的な状態のリセットなど |
...
| `unlisten(type: string, handler: EventListener)` | コンポーネントのルートノードからイベントリスナを削除する。`this.root_.removeEventListener` の単なる代替であることに注意。 |
